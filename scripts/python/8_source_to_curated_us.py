...
#get the session object and get dataframe
    ...
    sales_df = session.sql("select * from sales_dwh.source.us_sales_order")# apply filter to select only paid and delivered sale orders
    ...
    country_sales_df = shipped_sales_df.with_column('Country',lit('USA')).with_column('Region', lit('NA'))
    ...
    final_sales_df = final_sales_df.select(
        ...
        col('EU2USD').alias('Exhchange_Rate'),
        (col('FINAL_ORDER_AMOUNT')/col('EU2USD')).alias('EU_TOTAL_ORDER_AMT'),
        (col('TAX_AMOUNT')/col('EU2USD')).alias('EU_TAX_AMT'),
        col('payment_status'),
        col('shipping_status'),
        col('payment_method'),
    ...
    new_table_name = "SALES_DWH.CURATED.US_SALES_ORDER"
